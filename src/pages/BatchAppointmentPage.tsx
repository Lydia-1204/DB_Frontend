...
const BatchAppointmentPage: React.FC<Props> = ({ onBack, onAddAppointment }) => {
  ...
  return (
    <div className="page-wrapper"><div className="page-container"><div className="page-header"><div className="page-header-left"><button onClick={onBack} className="page-back-btn"><ArrowLeft className="w-6 h-6" /></button><div><h2 className="page-title">批量预约</h2></div></div><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><div style={{ width: '32px', height: '32px', background: 'rgba(255,255,255,0.2)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Users className="w-5 h-5" /></div><span style={{ color: 'rgba(255,255,255,0.9)', fontSize: '14px', fontWeight: 500 }}>当前 {batchForm.visitors.length} 人</span></div></div><div className="page-content"><div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '32px' }}>
  ...
  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', maxHeight: '400px', overflowY: 'auto' }}>
      ...
      <div className="content-section">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div><label className="form-label">老人ID (0表示全体)</label><input type="number" value={batchForm.elderlyId} onChange={e => setBatchForm(p => ({ ...p, elderlyId: Number(e.target.value) }))} className="form-input" placeholder="输入老人ID或0" /></div>
