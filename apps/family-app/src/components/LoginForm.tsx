...
// 统一“返回门户”跳转逻辑：本地尝试 localhost 可达性，远程直接跳远程
...
interface LoginFormProps {
  onLogin: (familyId: string, password: string) => Promise<boolean>;
}
...
export const LoginForm: React.FC<LoginFormProps> = ({ onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!familyId.trim() || !password.trim()) {
      setError('请输入家属ID和密码');
      return;
    }
    ...
    try {
      const success = await onLogin(familyId.trim(), password);
      
      if (!success) {
        setError('登录失败，请检查家属ID和密码');
      }
    } catch (error) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '2.5rem' }}>
          ...
          <p style={{ color: '#2563eb', fontSize: '1.125rem', marginTop: '0.5rem', margin: 0 }}>家属端登录</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2rem', width: '100%', maxWidth: '32rem' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
            <label htmlFor="familyId" style={{ marginBottom: '0.75rem', fontSize: '1.25rem', fontWeight: '600', color: '#1d4ed8', textAlign: 'center' }}>
              家属ID：
            </label>
            <input
              type="text"
        ...
        <div style={{ marginTop: '2.5rem', textAlign: 'center', fontSize: '1rem', color: '#2563eb', letterSpacing: '0.025em' }}>
          <p style={{ margin: 0 }}>💡 提示：请输入您的家属ID和密码进行登录</p>
        </div>
      </div>
    </div>
  );
